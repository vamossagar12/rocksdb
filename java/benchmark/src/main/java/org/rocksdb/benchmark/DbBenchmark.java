...
public class DbBenchmark {
    ...
    abstract public class BenchmarkTask implements Callable<Stats> {
        ...
        public BenchmarkTask(int tid, long numEntries) {
            tid_ = tid;
            numEntries_ = numEntries;
            stats_ = new Stats(tid);
        }
    ...
    abstract class WriteTask extends BenchmarkTask {
        ...
        public WriteTask(int tid, long numEntries, WriteOptions writeOpt, long entriesPerBatch) {
            super(tid, numEntries);
            writeOpt_ = writeOpt;
            entriesPerBatch_ = entriesPerBatch;
            maxWritesPerSecond_ = -1;
        }
    ...
    public DbBenchmark(Map<Flag, Object> flags) throws Exception {
        ...
        fileType_ = (String) flags.get(Flag.file_type);
        ...
        mergeKeys_ = (Integer) flags.get(Flag.merge_keys) == -1? num_: (Integer) flags.get(Flag.merge_keys);
    }
    ...
    private void prepareOptions(Options options) throws RocksDBException {
        ...
        if (flags_.get(Flag.merge_operator) != null && !flags_.get(Flag.merge_operator).toString().isEmpty()) {
            String mergeOperator = (String) flags_.get(Flag.merge_operator);
    ...
    */
    ...
    private void run() throws RocksDBException {
        ...
        for (String benchmark : benchmarks_) {
            ...
            switch (benchmark) {
                ...
                case "mergerandom":
                    if (flags_.get(Flag.merge_operator) == null || flags_.get(Flag.merge_operator).toString().isEmpty()) {
                        throw new IllegalArgumentException("Merge operator can't be empty for mergerrandom benchmark");
                    }
                    tasks.add(new MergeRandomTask(
                            currentTaskId++, randSeed_, num_, mergeKeys_, writeOpt, 1));
                    break;
    ...
    public void generateKeyFromLong(byte[] slice, int numKeys, long n) {
        assert(n >= 0);
        int startPos = 0;

        if (keysPerPrefix_ > 0) {
            long numPrefix = (numKeys + keysPerPrefix_ - 1) / keysPerPrefix_;
            long prefix = n % numPrefix;
            int bytesToFill = Math.min(prefixSize_, 8);
            for (int i = 0; i < bytesToFill; ++i) {
                slice[i] = (byte) (prefix % 256);
    ...
    private enum Flag {
        ...
        use_default_column_family(true,
                "If true, the benchmarks would be run against default column family, otherwise\n" +
                        "against the user specfied column families") {
            @Override
            public Object parseValue(String value) {
                return parseBoolean(value);
            }
        },
        file_type("catalog_dump",
                "Specify the file type to be used for writes") {
            @Override
            public Object parseValue(String value) {
                return value;
            }
